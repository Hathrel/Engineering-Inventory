...
namespace Engineering_Inventory
{
    public class PythonInterop
    {
        ...
        public void InitializePythonEngine()
        {
            ...
            string dataFilePath = Path.Combine(baseDirectory, "Data");
            string configFilePath = Path.Combine(dataFilePath, "config.json");
            
            Runtime.PythonDLL = Path.Combine(dataFilePath, "python-3.12.3-embed-amd64", "python312.dll");
            PythonEngine.PythonPath =
                Path.Combine(dataFilePath, "python-3.12.3-embed-amd64") + ";" +
                Path.Combine(dataFilePath, "python-3.12.3-embed-amd64", "python312") + ";" +
                Path.Combine(dataFilePath, "python-3.12.3-embed-amd64", "python312", "site-packages") + ";" +
                Path.Combine(dataFilePath, "python-3.12.3-embed-amd64", "python312", "site-packages", "EngInvDB");
;
            ...
            using (Py.GIL())
            {
                pythonModule = Py.Import("EngInvDB.EngInvDB");
        ...
        //{
        //        string baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
        //        string pythonEmbeddablePath = Path.Combine(baseDirectory, "Data", "python-3.12.3-embed-amd64");
        //        string pythonDLLPath = Path.Combine(pythonEmbeddablePath, "python312.dll");
        ...
        //            // Pass the config file path to the Python module
        //            PyObject globals = pythonModule.GetAttr("__dict__");
        //            globals.SetItem("config_path", new PyString(configFilePath));
        //        }
        //}
